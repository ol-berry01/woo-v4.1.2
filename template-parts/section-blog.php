<?php
$gcwoo_enable_section = get_theme_mod( 'gcwoo_blog_enable', true );
if ( $gcwoo_enable_section || is_customize_preview() ) :
?>
<div id="blog-section" class="blog-section" <?php if( false == $gcwoo_enable_section ): echo 'style="display: none;"'; endif ?>>
    <div class="title-section">
        <h3 class="section-title blog-title wow fadeIn"><?php echo esc_html( get_theme_mod( 'gcwoo_blog_title', __( 'Golf Car UK News', 'gcwoo' ) ) ); ?></h3>
    </div>
    	<?php
        $args = array(
            'post_type' => 'post',
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class('blog-item wow fadeIn'); ?>>
				...
				<div class="blog-item-image">
					<a href="<?php echo esc_url( get_permalink() ) ?>" class="ql_thumbnail_hover" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>">
						<?php the_post_thumbnail( 'gcwoo_blog-section' ); ?>
					</a>
				</div><!-- /post_image -->
				<?php } ?>
                ...
                <div class="blog-item-entry">
                	...
                	<?php the_excerpt(); ?>
                    <a href="<?php echo esc_url( get_permalink() ) ?>" class="read-more"><?php esc_html_e( 'Read More', 'gcwoo' ); ?> <i class="fa fa-angle-right"></i></a>
                </div>
            </article>
